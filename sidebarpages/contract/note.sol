...
contract Promissory{
    ...
    event logNewNote(address creditor, address debtor);
    ...
    function create_note(address creditor, address debtor, uint256 _hash) public {
        ...
        emit logNewNote(creditor, debtor);
    }
    
    function confirm_note(address creditor,uint256 _hash) public{
        ...
        if(notes[_hash].creditor_confirmed && notes[_hash].debtor_confirmed){
            emit logConfirmed(creditor, notes[_hash].note_hash);
        }
    }
    ...
    function check_note(uint256 _hash) public view returns (address number){
        ...
        if(notes[_hash].creditor_confirmed && notes[_hash].debtor_confirmed) number = notes[_hash].creditor_addr;
